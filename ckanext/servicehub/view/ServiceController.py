...
def report_action(app_id, action_name, call_id):
    ...
    report = session.query(AppTestReport).filter(
        AppTestReport.app_id == app_id and AppTestReport.call_id == call_id).first()
    if report == None:
...
def report_ajax(id):
    ...
    if service_ins.get('error', '') != '':
        base.abort(404, _(u'Service not found'))
    report_ins_lst = session.query(App, Call, AppTestReport).join(Call).join(AppTestReport,
                                                                             AppTestReport.call_id == Call.call_id).filter(
        App.app_id == id).all()
    report_lst = map(
        lambda report: [report[1].call_id,report[2].code_using, report[1].user_id, report[1].created_at, report[2].note
