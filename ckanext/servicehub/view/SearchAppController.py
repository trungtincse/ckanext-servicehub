...
blueprint = Blueprint('app', __name__, url_prefix='/app/search')


def register_rules():
    blueprint.add_url_rule(u'/', view_func=search_page, strict_slashes=False)
...
def search_page():
    ...
    data_dict = {
        ...
        'organization': request.params.getlist('organization'),
        'categories': request.params.getlist('categories'),
        'language': request.params.get('language'),
    }

    results = get_action('app_search')(context, data_dict)
    from pprint import pprint
    pprint(results)
    return base.render('search/index.html', dict(results=results))
...
@blueprint.route('/<string:id>/delete', methods=['GET', 'POST'])
def delete(id):
    context = {
        u'model': model,
        u'session': model.Session,
...
@blueprint.route('/<string:id>/monitor', methods=['GET'])
def monitor(id):
    extra_vars = {}
    context = {
        u'model': model,
...
@blueprint.route('/<string:id>/setting', methods=['POST'])
def setting(id):
    extra_vars = {}
    context = {
        u'model': model,
...
@blueprint.route('/<string:id>/avatar', methods=['GET'])
def getAvatar(id):
    extra_vars = {}
    context = {
        u'model': model,
...
@blueprint.route('/<string:id>/update', methods=['POST'])
def update(id):
    context = {
        u'model': model,
        u'session': model.Session,
